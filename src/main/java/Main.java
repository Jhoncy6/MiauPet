    ...
    public class Main {
        ...
        private static void listarAnimais_SelecionarAnimal(int idCliente) {
            ...
            int escolha = lerInteiro("Escolha o número do animal (ou 0 para voltar): ");

            if (escolha == 0) return;

            if (escolha > 0 && escolha <= animais.size()) {
                Animal animalSelecionado = animais.get(escolha - 1);
        ...
        private static void menuAnimalEscolhido(Animal animalSelecionado) {
            ...
            do {
                ...
                switch (opcaoMenuAnimal) {
                    case 1:
                        administrarConsultas(animalSelecionado);
                        break;
                    case 2:
        ...
        private static void administrarConsultas(Animal animal) {
            System.out.println("--------- Consultas de " + animal.getNome() + " ---------");
            List<Consulta> consultas = consultaDAO.buscarConsultasPorIdAnimal(animal.getId());

            if (consultas.isEmpty()) {
                System.out.println("Este animal não possui consultas registadas.");
                return;
            }

            for (int i = 0; i < consultas.size(); i++) {
                Consulta c = consultas.get(i);
                System.out.println((i + 1) + " - Data: " + c.getDia() + " | Motivo: " + c.getMotivo());
            }

            int escolha = lerInteiro("Escolha o numero da consulta para editar (ou 0 para voltar): ");

            if (escolha == 0) return;

            if (escolha > 0 && escolha <= consultas.size()) {
                Consulta consultaSelecionada = consultas.get(escolha - 1);
                editarConsulta(consultaSelecionada);
            } else {
                System.out.println("Opção inválida. Tente novamente.");
            }
        }

        private static void editarConsulta(Consulta consulta) {
            int opcao;
            do {
                System.out.println("------- Editando Consulta -------");
                System.out.println("Motivo: " + consulta.getMotivo());
                System.out.println("Comentários: " + consulta.getComentarios());

                System.out.println("Serviços Atuais: ");
                if(consulta.getServicos().isEmpty()) System.out.println(" - Nenhum serviço");
                for(Servico s : consulta.getServicos()) {
                    System.out.println(" - " + s.getNomeServico() + " (R$ " + s.getPreco() + ")");
                }

                System.out.println("----------------------------------------");
                System.out.println("1 - Alterar Motivo");
                System.out.println("2 - Alterar Comentários ");
                System.out.println("3 - Adicionar Novo Serviço");
                ...
                switch (opcao) {
                    case 1:
                        ...
                        System.out.println("(Alteração pendente. Escolha '9' para salvar)");
                        break;

                    case 2:
                        System.out.print("Novos Comentários: ");
                        String novosComents = scanner.nextLine();
                        consulta.setComentarios(novosComents);
                        System.out.println("(Alteração pendente. Escolha '9' para salvar)");
                        break;

                    case 3:
                        adicionarServicoNaConsultaExistente(consulta);
                        break;

                    case 4:
                        System.out.println("!!! PERIGO !!!");
                        System.out.println("Tem certeza que deseja apagar esta consulta? (1-Sim / 0-Não)");
                        int confirm = lerInteiro("Confirma: ");
                        if (confirm == 1) {
                            if (consultaDAO.removerConsulta(consulta.getId())) {
                    ...
                    case 9:
                        if (consultaDAO.atualizarConsulta(consulta)) {
                            System.out.println("Dados da consulta atualizados com sucesso!");
                            opcao = 0;
                        } else {
                    ...
                    case 0:
                        System.out.println("Voltando (alterações não salvas serão perdidas)...");
                        break;
        ...
        private static void adicionarServicoNaConsultaExistente(Consulta consulta) {
            System.out.println("--- Adicionar Serviço Extra ---");
            ...
            int idServico = lerInteiro("ID do serviço a adicionar: ");
            ...
            if (servicoEscolhido != null) {
                ...
                boolean inseriu = consultaDAO.adicionarServicoNaConsulta(consulta.getId(), servicoEscolhido.getId());

                if (inseriu) {
                    consulta.adicionarServico(servicoEscolhido);
                    System.out.println("Serviço adicionado com sucesso!");
                } else {
                    System.out.println("Erro ao adicionar serviço (pode ser que já esteja adicionado).");
                }
            } else {
        ...
        private static boolean editarAnimal(Animal animal) {
            int opcao;
            do {
                System.out.println("--- Editando: " + animal.getNome() + " ---");
                System.out.println("1 - Alterar Nome (" + animal.getNome() + ")");
                System.out.println("2 - Alterar Espécie (" + animal.getEspecie() + ")");
                System.out.println("3 - Alterar Raça (" + animal.getRaca() + ")");
                System.out.println("4 - EXCLUIR ANIMAL DO SISTEMA");
                System.out.println("9 - SALVAR ALTERAÇÕES");
                System.out.println("0 - Cancelar/Voltar");
                ...
                switch (opcao) {
                    ...
                    case 4:
                        ...
                        System.out.println("Ao remover o animal " + animal.getNome() + ", todas as consultas dele também serão apagadas.");
                        int confirmacao = lerInteiro("Digite 1 para CONFIRMAR a exclusão: ");
                        if (confirmacao == 1) {
                            if (animalDAO.removerAnimalPorId(animal.getId())) {
                        ...
                        break;
                    case 9:
                        if (animalDAO.atualizarAnimal(animal)) {
                            System.out.println("Dados do animal atualizados com sucesso!");
                            return false;
                        } else {
                            System.out.println("Erro ao atualizar no banco de dados.");
                        }
